...
The Context API allows us to share data considered "global" for a tree of React components.

Using context, we can avoid passing props through intermediate components.
...
In order to create a custom hook we only need to declare a function, and return a object or an array.
...
The state inside of a custom hook is isolated, that means
if you use the same hook in two different components, the
